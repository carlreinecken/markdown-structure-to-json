const parser = (text) => {
  ...
  function makeBlockList (lines) {
    ...
    // it may happen that in the markdown there is no header...
    if (lines.length < 1) {
      return result
    }
    ...
    const block = {
      header: header.replace(HEADER_REGEX, ''),
      content: []
    }
    ...
    block.content = makeBlockList(block.content)

    result.push(block)

    // all the lines after the block have to be parsed as well
    makeBlockList(lines).map(blck => result.push(blck))
